...
picks the **headliner** — the single highest-TM *designable* design across all λ (adopts the recognizable
...
def main() -> None:
    ...
    print(f"{'fold':<16}{'pdb':<6}{'len':>4} {'base d/TM':>10} | "
          + " | ".join(f"λ{l:g} d/TM".rjust(11) for l in LAMS) + " |  HEADLINER (λ, TM, scRMSD)")
    ...
    for jf in sorted(glob.glob(os.path.join(a.results_dir, a.variant, "*.json"))):
        ...
        # headliner: highest-TM DESIGNABLE design across all spa λ
        best = None
        for l in LAMS:
            for e in by.get(("spa", l), []):
                if e["des"] and (best is None or e["tm"] > best["tm"]):
                    best = {**e, "lam": l}
        hl = f"λ{best['lam']:g}, TM {best['tm']:.2f}, scRMSD {best['scrmsd']:.2f}Å" if best else "none designable"
        bs = f"{b[0]:.2f}/{b[1]:.2f}" if b else "—"
        print(f"{m.get('fold_name', pid):<16}{pid:<6}{m.get('len', '?'):>4} {bs:>10} | " + " | ".join(cells) + f" |  {hl}")
    ...
    print("\n(d = designable rate scRMSD<cut over 8 designs; TM = mean prompt-adherence to the named fold; "
          "headliner = highest-TM design that is also designable — the poster candidate.)")
    if a.out:
