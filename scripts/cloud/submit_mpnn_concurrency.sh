...
#
# Usage:
#   DRY_RUN=1 bash scripts/cloud/submit_mpnn_concurrency.sh
#   bash scripts/cloud/submit_mpnn_concurrency.sh
set -euo pipefail
...
DISK_GB="${DISK_GB:-150}"
NAME="${NAME:-spa-mpnn-conc-$(date -u +%Y%m%d-%H%M%S)}"
GCLOUD="${GCLOUD:-/home/user1/google-cloud-sdk/bin/gcloud}"

. "$(dirname "${BASH_SOURCE[0]}")/_pin_run_env.sh"

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNNER_LOCAL="${HERE}/run_mpnn_concurrency.sh"
BUCKET_NAME="${BUCKET#gs://}"
RUNNER_GCS="${BUCKET}/eval/mpnn_concurrency/runner-$(date -u +%Y%m%d-%H%M%S).sh"
