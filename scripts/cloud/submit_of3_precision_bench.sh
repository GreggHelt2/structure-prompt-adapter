...
#
# Usage:
#   DRY_RUN=1 bash scripts/cloud/submit_of3_precision_bench.sh
#   bash scripts/cloud/submit_of3_precision_bench.sh
set -euo pipefail
...
DISK_GB="${DISK_GB:-150}"
NAME="${NAME:-spa-of3-precision-$(date -u +%Y%m%d-%H%M%S)}"
GCLOUD="${GCLOUD:-/home/user1/google-cloud-sdk/bin/gcloud}"

. "$(dirname "${BASH_SOURCE[0]}")/_pin_run_env.sh"

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNNER_LOCAL="${HERE}/run_of3_precision_bench.sh"
BUCKET_NAME="${BUCKET#gs://}"
RUNNER_GCS="${BUCKET}/eval/of3_precision_bench/runner-$(date -u +%Y%m%d-%H%M%S).sh"
